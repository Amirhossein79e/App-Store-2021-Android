...
class MainRecyclerAdapter(private val context:Context, private val list: List<HomeCategoryModel>, private val callback:Callback) : RecyclerView.Adapter<MainRecyclerAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(position % 2 == 0)
        {
            ...
            (holder.recycler.layoutParams as FrameLayout.LayoutParams).marginStart = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,16f,metrics).toInt()
        ...
        }else
        {
            ...
            (holder.recycler.layoutParams as FrameLayout.LayoutParams).marginEnd = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,16f,metrics).toInt()
            cardParams.width = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,0f,metrics).toInt()
